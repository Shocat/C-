...
{
    public class gruz_carController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            {
                connection.Open();
                using (var command = new NpgsqlCommand("SELECT cars.*, content.* FROM cars INNER JOIN content ON cars.id = content.id WHERE cars.category = 'грузовой'", connection))
                {

                    var models = new List<gruzcar>();
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var model = new gruzcar
                            {

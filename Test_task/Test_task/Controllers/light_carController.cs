...
{
    public class light_carController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            {
                connection.Open();
                using (var command = new NpgsqlCommand("SELECT cars.*, content.* FROM cars INNER JOIN content ON cars.id = content.id WHERE cars.category = 'легковой'", connection))
                 {

                    var models = new List<lightcar>();
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var model = new lightcar
                            {
